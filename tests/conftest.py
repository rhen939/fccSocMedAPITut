...
def test_user2(client):
    user_data = {"email": "hello4@example.com", "password": "password"}
    res = client.post("/users/", json=user_data)

    assert res.status_code == 201
    new_user = res.json()
    assert new_user['email'] == "hello4@example.com"
...
def test_user(client):
    user_data = {"email": "hello3@example.com", "password": "password"}
    res = client.post("/users/", json=user_data)

    assert res.status_code == 201
    new_user = res.json()
    assert new_user['email'] == "hello3@example.com"
